...
后付费主机停机时不收取主机CPU、内存和网络增强的费用。其他付费类型的主机停机时仍会照常收费，请确保将不使用的主机删除。
...
首先定义下各个数据中心的KMS地址（$kms\_name）。步骤4中会用到$kms\_name，请用相应的地址替代。例如华北（北京2）可用区E的地址为hb06.kms.ucloud.cn。若需其他可用区地址，请联系技术支持。
