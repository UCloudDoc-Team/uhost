...
创建UCloud云主机时，用户可根据需要部署的应用类型和规模选择不同类型的云主机。目前，UCloud CPU云主机提供以下机型：快杰O型、快杰PRO型、快杰内存型、快杰共享型、通用型。不同机型提供不同的计算，内存，存储和网络带宽搭配，用户可灵活地为业务场景选择与之相匹配的云主机。
...
| ------------- | ---------- | ------------------------------------------------------------ | -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 快杰O型       | 快杰系列   | Intel CascadeLake: 2.5GHz-3.9GHz <br> Intel CascadelakeR: 3.0GHz-4.0GHz <br> Intel IceLake: 2.6GHz-3.4GHz <br> **Intel EmeraldRapids(新增): 2.7GHz-4.0GHz** <br> AMD EPYC2: 2.9GHz-3.4GHz <br> Ampere Altra: 3.0GHz | 25GbE    | 提供基于x86和Arm两种架构的CPU平台可选 <br> 计算、存储、网络性能卓越 | 全面场景                                                     |
...
| 快杰内存型    | 快杰系列   | Intel CascadeLake: 2.5GHz-3.9GHz <br/> Intel CascadelakeR: 3.0GHz-4.0GHz <br/>Intel IceLake:2.6GHz-3.4GHz | 25GbE    | 采用大容量内存型服务器，提供超大cpu内存配比选择              | 内存型数据库，例如：Redis <br> Hadoop集群，Spark集群以及其他企业大内存需求应用 |
| 快杰共享型    | 快杰系列   | Intel CascadeLake: 2.5GHz-3.9GHz <br/> Intel CascadelakeR: 3.0GHz-4.0GHz <br/> Intel IceLake: 2.6GHz-3.4GHz <br/>AMD EPYC2: 2.6GHz-3.3GHz | 25GbE    | CPU资源性能共享搭配华北（乌兰察布）自建机房提供较高性价比的云计算服务 | 中小型网站，轻量级数据库，轻量级企业应用等                   |
| 通用型 N      | 上一代系列 | Intel Skylake: 2.6GHz-3.7GHz <br> Intel Broadwell: 2.2GHz-2.9GHz | 10GbE    | 配置自由灵活，选择丰富                                       | 企业级应用，内存服务，数据分析等                             |
| 高主频型 C    | 上一代系列 | Intel Skylake:  3.2GHz-4.2GHz                                | 10GbE    | 采用3.2GHz主频的CPU，单核计算性能较高                        | 高频交易，数据处理，EDA等                                    |

**快杰O型规格族对应详情：**

| 规格族  | CPU厂商            | CPU平台                                                      | 与原O机型对比            |
| ------- | ------------------ | ------------------------------------------------------------ | ------------------------ |
| o1i     | Intel (x86_64)     | Intel CascadeLake: 2.5GHz \| 3.9GHz <br>Intel CascadelakeR: 3.0GHz \| 4.0GHz <br>Intel IceLake: 2.6GHz \| 3.4GHz | 原O机型Intel (x86_64)    |
| o1a     | AMD (x86_64)       | AMD EPYC2: 2.9GHz \| 3.4GHz                                  | 原O机型AMD (x86_64)      |
| o1r     | Ampere（AArch64）  | Ampere Altra: 3.0GHz                                         | 原O机型Ampere（AArch64） |
...
- 规格族定义：原快杰O云主机定义为o1族与o2族，o2族将提供较o1族更新一代的服务器。目前仅部分可用区上线，不同规格族价格存在不同，具体请参考控制台。
...
2）CPU平台支持（区分规格族售卖）：Intel Cascadelake(Gold6248)；Intel Cascadelake-Refresh(Gold6248R)；Intel IceLake(8358P); Intel EmeraldRapids(8558P); AMD EPYC2(Rome7542)；EPYC2(Rome 7H12)；Ampere Altra(Q80-30)。

3）CPU内存组合（支持配比1:1-1:8，不同可用区、不同规格族可选配比不同，具体配比以创建控制台为准）：
...
- 限制：网络增强2.0特性仅支持部分高内核版本镜像（即内核版本≥4.19的镜像）。

### 快杰PRO 通用型 

1）概述： <br> 快杰PRO 通用型云主机是独享vCPU的机型，云主机间无CPU资源竞争问题，持续提供稳定且高质量的计算性能。

2）CPU平台支持：Intel Cascadelake(Gold6248)；Intel Cascadelake-Refresh(Gold6248R)；Intel IceLake(8358P); AMD EPYC2(Rome 7542)；AMD EPYC2(Rome 7H12)。
...
2）CPU平台支持：AMD EPYC2(Rome7542)；AMD EPYC2(Rome 7H12)。
...
1）概述：<br> 快杰共享型采用非绑定CPU调度模式。在CPU资源共享的基础上，配合华北（乌兰察布）自建机房和性价比俱佳的AMD服务器，用户可在非性能敏感型场景下，用低价格享受优质的云计算服务。

2）CPU平台支持：Intel Cascadelake(Gold6248)；Intel Cascadelake-Refresh(Gold6248R)；Intel IceLake(8358P); AMD EPYC2(Rome7542)；EPYC2(Rome 7H12)。
