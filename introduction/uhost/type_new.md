...
创建Ucloud云服务器时，用户可根据需要部署的应用类型和规模选择不同类型的云服务器。目前，UCloud云服务器提供以下7类机型：快杰O型、快杰PRO型、快杰内存型、快杰共享型、通用型、高主频型、GPU型。不同机型提供不同的计算，内存，存储和网络带宽搭配，用户可灵活地为业务场景选择与之相匹配的云服务器。
...
快杰O型云服务器在Web服务，游戏服务，数据分析处理等绝大多数场景表现出色。
...
1）概述：快杰PRO型采用AMD EPYC 7F52 CPU（3.5GHz-3.9GHz），在快杰系列中享最高单核算力。同时，内存带宽达快杰O型的2倍，平均每核L3缓存达快杰型O的4倍，最大网络性能达到1000W PPS，最大存储性能达到120W IOPS。 
...
1）概述：共享型采用非绑定CPU调度模式。在CPU资源共享的基础上，配合乌兰察布自建机房和性价比俱佳的AMD服务器，用户可在非性能敏感型场景下，用最低成本享受优质的云计算服务。
